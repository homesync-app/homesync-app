-- Reconstructed from remote migration history (version 20260304122816).
-- Source: supabase_migrations.schema_migrations on project tfavamqszdkoeabpyxms.

DROP FUNCTION IF EXISTS public.get_filtered_expenses(uuid, text, text, int, int);

-- 1. Improved Filtered Expenses RPC (with Join and User Filtering)
...
BEGIN
  ...
  FROM (
    ...
    WHERE e.household_id = p_household_id
      AND (p_type = 'all' OR e.type = p_type)
      AND (
        p_sharing = 'all' 
