...
public abstract class DecoderVideoRenderer extends BaseRenderer {
    ...
    protected void onInputFormatChanged(FormatHolder formatHolder) throws ExoPlaybackException {
        ...
        if (this.sourceDrmSession != this.decoderDrmSession) {
            decoderReuseEvaluationCanReuseDecoder = new DecoderReuseEvaluation(this.decoder.getName(), (Format) Preconditions.checkNotNull(format2), format, 0, 128);
        } else {
            decoderReuseEvaluationCanReuseDecoder = canReuseDecoder(decoder.getName(), (Format) Preconditions.checkNotNull(format2), format);
        }
        ...
        this.eventDispatcher.inputFormatChanged((Format) Preconditions.checkNotNull(this.inputFormat), decoderReuseEvaluationCanReuseDecoder);
    }
    ...
    private void maybeInitDecoder() throws ExoPlaybackException {
        ...
        try {
            ...
            Decoder<DecoderInputBuffer, ? extends VideoDecoderOutputBuffer, ? extends DecoderException> decoderCreateDecoder = createDecoder((Format) Preconditions.checkNotNull(this.inputFormat), cryptoConfig);
            this.decoder = decoderCreateDecoder;
            decoderCreateDecoder.setOutputStartTimeUs(getLastResetPositionUs());
            setDecoderOutputMode(this.outputMode);
            long jElapsedRealtime2 = SystemClock.elapsedRealtime();
            this.eventDispatcher.decoderInitialized(((Decoder) Preconditions.checkNotNull(this.decoder)).getName(), jElapsedRealtime2, jElapsedRealtime2 - jElapsedRealtime);
