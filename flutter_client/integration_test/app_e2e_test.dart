...
void main() {
  ...
  testWidgets('Flujo E2E: Iniciar sesión, ver tareas e intentar canjear recompensa', (WidgetTester tester) async {
    ...
    if (loginTitleFinder.evaluate().isNotEmpty) {
      ...
      await tester.enterText(find.widgetWithText(TextFormField, 'Correo electrónico'), 'tu_pareja@gmail.com');
    ...
    expect(find.text('Inicio'), findsWidgets, reason: 'Deberíamos haber navegado al HomeScreen principal');
    ...
    final canjearFinder = find.text('Canjear').first; // Busca el primer botón de canjear disponible
    if (canjearFinder.evaluate().isNotEmpty) {
      ...
      await tester.pumpAndSettle(const Duration(seconds: 4)); // Espera petición al servidor
      ...
      expect(snackBarText, findsOneWidget, reason: 'Se esperaba un mensaje Toast (SnackBar) de confirmación/error de Supabase');
    }
  });
}
