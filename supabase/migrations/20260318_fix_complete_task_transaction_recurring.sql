-- Allow recurring tasks already verified in previous days to be completed again.
-- This fixes "today scheduled task" flows when the task was verified in a prior cycle.
...
BEGIN
  SELECT description, category, completed_at
    INTO v_task_desc, v_task_cat, v_completed_at
  FROM public.tasks
  WHERE id = p_task_id
    AND household_id = p_household_id
    AND (
      status IN ('assigned', 'active', 'in_progress', 'objected')
      OR (
        status = 'verified'
        AND recurrence_type IS NOT NULL
        AND COALESCE(next_due_at, due_at, created_at) <= NOW()
      )
    )
  FOR UPDATE;
  ...
  IF v_completed_at IS NOT NULL AND DATE(v_completed_at) = CURRENT_DATE THEN
    RETURN jsonb_build_object(
      'success', false,
  ...
  VALUES (
    ...
    'task',
    p_task_id,
    jsonb_build_object('status', 'pending_verification', 'activity_id', v_activity_id, 'performers', p_user_ids),
    'Completed',
    'rpc'
  );
