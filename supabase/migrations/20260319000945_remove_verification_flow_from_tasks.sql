-- Reconstructed from remote migration history (version 20260319000945).
-- Source: supabase_migrations.schema_migrations on project tfavamqszdkoeabpyxms.

-- Remove verification gating from task completion flow.
-- Goal: any existing task can be completed directly, repeatedly, without pending_verification states.
...
BEGIN
  ...
  RETURN v_result || jsonb_build_object('activity_id', v_activity_id, 'task_status', 'active');
END;
$$;

-- Normalize legacy statuses so tasks stop disappearing from selectors.
UPDATE public.tasks
SET
  status = 'active',
  updated_at = NOW()
WHERE status IN ('pending_approval', 'pending_verification', 'verified');
