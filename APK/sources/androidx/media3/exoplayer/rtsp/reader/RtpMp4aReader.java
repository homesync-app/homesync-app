...
final class RtpMp4aReader implements RtpPayloadReader {
    ...
    private static int getNumOfSubframesFromMpeg4AudioConfig(ImmutableMap<String, String> immutableMap) throws ParserException {
        ...
        if (str != null && str.length() % 2 == 0) {
            ...
            if (bits == 0) {
                ...
                Preconditions.checkArgument(parsableBitArray.readBits(4) == 0, "Only suppors one program.");
                Preconditions.checkArgument(parsableBitArray.readBits(3) == 0, "Only suppors one layer.");
                i = bits2;
            } else {
    ...
    private void outputSampleMetadataForFragmentedPackets() {
        ((TrackOutput) Preconditions.checkNotNull(this.trackOutput)).sampleMetadata(this.fragmentedSampleTimeUs, 1, this.fragmentedSampleSizeBytes, 0, null);
        this.fragmentedSampleSizeBytes = 0;
        this.fragmentedSampleTimeUs = C.TIME_UNSET;
    }
}
