...
final class RtpDataLoadable implements Loader.Loadable {
    ...
    public void setTimestamp(long j) {
        if (j == C.TIME_UNSET || ((RtpExtractor) Preconditions.checkNotNull(this.extractor)).hasReadFirstRtpPacket()) {
            return;
        }
    ...
    public void setSequenceNumber(int i) {
        if (((RtpExtractor) Preconditions.checkNotNull(this.extractor)).hasReadFirstRtpPacket()) {
            return;
        }
        ...
        try {
            if (this.dataChannel == null) {
                ...
                this.playbackThreadHandler.post(new Runnable() { // from class: androidx.media3.exoplayer.rtsp.RtpDataLoadable$$ExternalSyntheticLambda0
                    ...
                    public final void run() {
                        this.f$0.m268lambda$load$0$androidxmedia3exoplayerrtspRtpDataLoadable(transport, rtpDataChannel);
                    }
                });
                this.extractorInput = new DefaultExtractorInput((DataReader) Preconditions.checkNotNull(this.dataChannel), 0L, -1L);
            ...
            while (!this.loadCancelled) {
                if (this.pendingSeekPositionUs != C.TIME_UNSET) {
                    ((RtpExtractor) Preconditions.checkNotNull(this.extractor)).seek(this.nextRtpTimestamp, this.pendingSeekPositionUs);
                    this.pendingSeekPositionUs = C.TIME_UNSET;
                }
                if (((RtpExtractor) Preconditions.checkNotNull(this.extractor)).read((ExtractorInput) Preconditions.checkNotNull(this.extractorInput), new PositionHolder()) == -1) {
                    break;
                }
            }
            this.loadCancelled = false;
        } finally {
            if (((RtpDataChannel) Preconditions.checkNotNull(this.dataChannel)).needsClosingOnLoadCompletion()) {
    ...
    /* JADX INFO: renamed from: lambda$load$0$androidx-media3-exoplayer-rtsp-RtpDataLoadable, reason: not valid java name */
    /* synthetic */ void m268lambda$load$0$androidxmedia3exoplayerrtspRtpDataLoadable(String str, RtpDataChannel rtpDataChannel) {
        this.eventListener.onTransportReady(str, rtpDataChannel);
    }

    public void resetForSeek() {
        ((RtpExtractor) Preconditions.checkNotNull(this.extractor)).preSeek();
    }
