create or replace function public.qa_admin_update_task_v1(
  ...
  p_touch_completion boolean default false
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if p_completed_by is not null and not exists (
    ...
    raise exception 'El miembro que completa no pertenece al hogar QA seleccionado';
  end if;
  ...
  set
    ...
    completed_by = case
      when p_touch_completion then p_completed_by
      else completed_by
    end,
    completed_at = case
      when p_touch_completion then p_completed_at
      else completed_at
    end,
    last_completed_at = case
      when p_touch_completion then p_last_completed_at
...
grant execute on function public.qa_admin_update_task_v1(
  uuid, uuid, text, text, text, uuid, timestamptz, text, integer, integer[],
  integer[], text, uuid, timestamptz, timestamptz, boolean
) to authenticated;
