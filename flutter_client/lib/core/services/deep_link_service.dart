    ...
    _linkSubscription = _appLinks.uriLinkStream.listen((uri) {
      ...
      if (uri.host == 'auth-complete' || uri.path.contains('auth-complete')) {
        ...
        if (status == 'success') {
          ...
          showToast('❌ Error al conectar: ${message ?? "Desconocido"}', AppColors.error);
        }
      }

      // 2. Mercado Pago Payment callbacks
      if (uri.host == 'payment-success' || uri.path.contains('payment-success')) {
        showToast('🎉 ¡Acreditado! Se verá reflejado en unos segundos.', AppColors.success);
      ...
      if (uri.host == 'payment-pending' || uri.path.contains('payment-pending')) {
        showToast('⏳ Pago en proceso. Te avisaremos al acreditarse.', Colors.orange);
      }
    });
  }
