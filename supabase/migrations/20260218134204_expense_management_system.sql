-- Reconstructed from remote migration history (version 20260218134204).
...
INSERT INTO public.expense_categories (id, name, icon, color) VALUES
  ('groceries', 'Supermercado', 'Â­Æ’Ã¸Ã†', '#10B981'),
  ('utilities', 'Servicios', 'Â­Æ’Ã†Ã­', '#F59E0B'),
  ('rent', 'Alquiler', 'Â­Æ’Ã…Ã¡', '#3B82F6'),
  ('dining', 'Restaurantes', 'Â­Æ’Ã¬Â¢Â´Â©Ã…', '#EC4899'),
  ('transport', 'Transporte', 'Â­Æ’ÃœÃ¹', '#8B5CF6'),
  ('entertainment', 'Entretenimiento', 'Â­Æ’Ã„Â¼', '#06B6D4'),
  ('health', 'Salud', 'Â­Æ’Ã†Ã¨', '#EF4444'),
  ('other', 'Otros', 'Â­Æ’Ã´Âª', '#6B7280')
...
BEGIN
  ...
  IF p_split_user_ids IS NOT NULL THEN
    -- Split only with specified users
    v_member_ids := p_split_user_ids;
  ELSE
    -- Split with all household members
...
BEGIN
  ...
  LEFT JOIN owes o ON o.user_id = u.id
  WHERE u.id IN (
...
-- ============================================
-- RPC: SETTLE DEBT (Create expense to settle)
-- ============================================
...
DECLARE
  v_expense_id UUID;
BEGIN
  -- Create a settlement expense (only debtor owes, paid by creditor)
  INSERT INTO public.expenses (
    id,
    household_id,
    created_by_id,
    ...
    p_household_id,
    p_user_id,
    'Liquidaciâ”œâ”‚n de deuda',
    'Pago de deuda entre miembros',
    'other',
    p_amount,
    'EUR',
    p_to_user_id,  -- Creditor receives payment
    NOW()
  )
  RETURNING id INTO v_expense_id;

  -- Create single split (only debtor owes this)
  INSERT INTO public.expense_splits (
    id,
    expense_id,
    user_id,
...
GRANT EXECUTE ON FUNCTION public.create_expense(
  UUID, UUID, TEXT, DECIMAL, UUID, TEXT, TEXT, TEXT, TEXT, UUID[]
) TO authenticated;
