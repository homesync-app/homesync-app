-- Reconstructed from remote migration history (version 20260220115752).
...
BEGIN
  ...
  IF (TG_OP = 'INSERT' AND NEW.assigned_to IS NOT NULL AND NEW.assigned_to != NEW.created_by_id) OR
    ...
    VALUES (
      ...
      COALESCE(creator_name, 'Alguien') || ' te asignâ”œâ”‚ la tarea: ' || NEW.title,
      'task_assigned',
      'task',
      NEW.id
    );
  ...
  IF (TG_OP = 'UPDATE' AND NEW.status IN ('pending_verification', 'verified') AND OLD.status NOT IN ('pending_verification', 'verified')) THEN
    ...
    IF auth.uid() IS NULL OR NEW.created_by_id != auth.uid() THEN  
      ...
      VALUES (
        ...
        auth.uid(),
        'Tarea Completada',
        COALESCE(assignee_name, 'Alguien') || ' completâ”œâ”‚: ' || NEW.title,
        'task_completed',
        'task',
        NEW.id
      );
...
BEGIN
  ...
  LOOP
    ...
    VALUES (
      ...
      NEW.created_by_id,
      'Nuevo Gasto',
      COALESCE(creator_name, 'Alguien') || ' agregâ”œâ”‚ un gasto de ' || NEW.currency || ' ' || NEW.amount || ' por ' || NEW.title,
      'expense_added',
      'expense',
      NEW.id
    );
