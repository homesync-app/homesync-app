-- Preserve the user-selected completion date in task activity rows.
--
-- `complete_task_v1` already stores p_completed_at on tasks, but the activity
-- row still used its default created_at=now() and omitted completed_at from
-- metadata. Feed details opened from that activity then looked like the task
-- had been completed "today" even when the user chose an earlier day.
