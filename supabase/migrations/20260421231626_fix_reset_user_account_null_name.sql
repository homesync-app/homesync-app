-- Reconstructed from remote migration history (version 20260421231626).
...
BEGIN
  ...
  -- Reset profile: clear avatar and alias but keep the name so the user
  -- doesn't have to re-enter it during setup. Set name to NULL so the
  -- setup screen can re-collect it cleanly (UI shows "Usuario" as fallback).
  UPDATE public.users
  SET full_name = NULL,
      avatar_url = NULL,
