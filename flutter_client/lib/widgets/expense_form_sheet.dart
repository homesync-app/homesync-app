...
class _ExpenseFormSheetState extends ConsumerState<ExpenseFormSheet> {
  ...
  final Set<ShoppingItem> _selectedShoppingItems = {};
  ...
  final List<Map<String, dynamic>> _categories = [
    ...
    {'id': 'restaurants', 'name': 'Restaurantes', 'icon': '🍽️', 'color': Color(0xFFF06292)},
    {'id': 'transport', 'name': 'Transporte', 'icon': '🚗', 'color': Color(0xFF4DB6AC)},
    {'id': 'entertainment', 'name': 'Entretenimiento', 'icon': '🎬', 'color': Color(0xFF9575CD)},
  ...
  void _onTitleChanged() {
    if (widget.expenseToEdit != null) return; // Don't auto-suggest if editing
  ...
  void _matchAndSetCategory(String t) {
    t = t.toLowerCase();
    String? matchedId;
    if (t.contains('supermercado') || t.contains('coto') || t.contains('carrefour') || t.contains('despensa') || t.contains('comida') || t.contains('alimento')) matchedId = 'supermarket';
  ...
  void _loadExpenseData(Map<String, dynamic> exp) {
    ...
    if (exp['paid_at'] != null) {
      _selectedDate = DateTime.parse(exp['paid_at']);
    }
    if (exp['paid_by'] != null) {
      _paidByUserId = exp['paid_by'];
    }
    if (exp['split_type'] != null) {
      _splitMode = SplitType.values.firstWhere(
        (e) => e.name == exp['split_type'],
        orElse: () => SplitType.equal,
      );
    }
    
    if (exp['expense_splits'] != null) {
      final splits = exp['expense_splits'] as List;
    ...
    try {
      ...
      if (_selectedShoppingItems.isNotEmpty) {
        ...
        for (final item in _selectedShoppingItems) {
          await shoppingRepo.toggleItem(item.id, true);
        }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(widget.expenseToEdit != null ? 'Gasto actualizado' : 'Gasto guardado'),
    ...
    return membersAsync.when(
      ...
      data: (members) {
        if (_paidByUserId.isEmpty) {
          ...
          if (_selectedMembersForSplit.isEmpty && widget.expenseToEdit == null) {
  ...
  Future<void> _confirmDelete() async {
    ...
    if (confirm == true) {
      setState(() => _isLoading = true);
      try {
        await ref.read(expenseRepositoryProvider).deleteExpense(widget.expenseToEdit!['id']);
  ...
  Widget _buildDateAndPayerRow(BuildContext context, Member payer, List<Member> members) {
    return Row(
      children: [
        Expanded(
  ...
  void _showMemberSelector(BuildContext context, List<Member> members) {
        ...
        child: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: (_selectedCategory!['color'] as Color).withOpacity(0.1),
  ...
  Widget _buildShoppingIntegration(BuildContext context, AsyncValue<List<ShoppingItem>> shoppingItemsAsync) {
  ...
  void _showShoppingItemsSelector(BuildContext context, List<ShoppingItem> items) {
    showModalBottomSheet(
      context: context,
                  ...
                  Padding(
                    ...
                    child: SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () => Navigator.pop(context),
                        style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, shape: StadiumBorder(), elevation: 0),
  ...
  Widget _buildSplitConfiguration(BuildContext context, List<Member> members) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget _buildSplitDetails(List<Member> members) {
    if (_splitMode == SplitType.equal) {
      return Wrap(
        spacing: 8,
  ...
  Widget _buildInfoBox(String text, Color color) {
    return Container(
      ...
      child: Text(text, style: TextStyle(color: color.withOpacity(0.8), fontSize: 13, fontWeight: FontWeight.w500)),
    );
  }
