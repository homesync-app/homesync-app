    ...
    public final void getNextChunk(LoadingInfo loadingInfo, long j, List<? extends MediaChunk> list, ChunkHolder chunkHolder) {
        ...
        if (this.cmcdConfiguration != null) {
            ...
            if (i4 < streamElement.chunkCount) {
                chunkDurationUs.setNextObjectRequest(UriUtil.getRelativePath(uriBuildRequestUri, streamElement.buildRequestUri(indexInTrackGroup, i4)));
            }
        } else {
            chunkDurationUs = null;
        }
        this.lastChunkRequestRealtimeMs = SystemClock.elapsedRealtime();
        chunkHolder.chunk = newMediaChunk(this.trackSelection.getSelectedFormat(), this.dataSource, uriBuildRequestUri, i3, startTimeUs, chunkDurationUs2, j4, this.trackSelection.getSelectionReason(), this.trackSelection.getSelectionData(), chunkExtractor, chunkDurationUs);
    }
