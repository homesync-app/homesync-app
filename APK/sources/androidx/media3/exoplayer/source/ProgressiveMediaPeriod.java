...
final class ProgressiveMediaPeriod implements MediaPeriod, ExtractorOutput, Loader.Callback<ExtractingLoadable>, Loader.ReleaseCallback, SampleQueue.UpstreamFormatChangedListener {
    ...
    public void prepare(MediaPeriod.Callback callback, long j) {
        ...
        if (this.singleTrackFormat != null) {
            ...
            m289x33ac0ff2(new IndexSeekMap(new long[]{0}, new long[]{0}, C.TIME_UNSET));
            endTracks();
    ...
    public long selectTracks(ExoTrackSelection[] exoTrackSelectionArr, boolean[] zArr, SampleStream[] sampleStreamArr, boolean[] zArr2, long j) {
        ...
        for (int i3 = 0; i3 < exoTrackSelectionArr.length; i3++) {
            ...
            if (sampleStream != null && (exoTrackSelectionArr[i3] == null || !zArr[i3])) {
                ...
                Preconditions.checkState(zArr3[i4]);
        ...
        for (int i5 = 0; i5 < exoTrackSelectionArr.length; i5++) {
            if (sampleStreamArr[i5] == null && (exoTrackSelection = exoTrackSelectionArr[i5]) != null) {
                Preconditions.checkState(exoTrackSelection.length() == 1);
                Preconditions.checkState(exoTrackSelection.getIndexInTrackGroup(0) == 0);
                int iIndexOf = trackGroupArray.indexOf(exoTrackSelection.getTrackGroup());
                Preconditions.checkState(!zArr3[iIndexOf]);
                this.enabledTrackCount++;
                zArr3[iIndexOf] = true;
        ...
        if (this.loadOnlySelectedTracks) {
            ...
            while (true) {
                ...
                if (i6 >= controlledTrackOutputArr.length) {
                    break;
                }
                controlledTrackOutputArr[i6].updateSelectionState(zArr3[i6]);
                i6++;
            }
        }
    ...
    public void seekMap(final SeekMap seekMap) {
        this.handler.post(new Runnable() { // from class: androidx.media3.exoplayer.source.ProgressiveMediaPeriod$$ExternalSyntheticLambda3
            @Override // java.lang.Runnable
            public final void run() {
                this.f$0.m289x33ac0ff2(seekMap);
            }
        });
    }
    ...
    public void m289x33ac0ff2(SeekMap seekMap) {
    ...
    public void maybeFinishPrepare() {
        ...
        for (int i = 0; i < length; i++) {
            Format formatBuild = (Format) Preconditions.checkNotNull(this.sampleQueues[i].getUpstreamFormat());
    ...
    private void startLoading() {
        ...
        if (this.prepared) {
            ...
            extractingLoadable.setLoadPosition(((SeekMap) Preconditions.checkNotNull(this.seekMap)).getSeekPoints(this.pendingResetPositionUs).first.position, this.pendingResetPositionUs);
            for (SampleQueue sampleQueue : this.sampleQueues) {
    ...
    private boolean seekInsideBufferUs(boolean[] zArr, long j, boolean z) throws Throwable {
        ...
        for (int i = 0; i < length; i++) {
            SampleQueue sampleQueue = this.sampleQueues[i];
            if (this.controlledTrackOutputs[i].isSelected() && (sampleQueue.getReadIndex() != 0 || !z)) {
                if (this.isSingleSample) {
                    zSeekTo = sampleQueue.seekTo(sampleQueue.getFirstIndex());
                } else {
                    zSeekTo = sampleQueue.seekTo(j, this.loadingFinished);
                }
    ...
    public long getLargestQueuedTimestampUs(boolean z) {
        ...
        for (int i = 0; i < this.sampleQueues.length; i++) {
            if (z || ((TrackState) Preconditions.checkNotNull(this.trackState)).trackEnabledStates[i]) {
    ...
    final class ExtractingLoadable implements Loader.Loadable, IcyDataSource.Listener {
        ...
        public void load() throws IOException {
            ...
            while (i == 0 && !this.loadCanceled) {
                try {
                    ...
                    DataSpec dataSpecBuildDataSpec = buildDataSpec(j, str);
        ...
        public void onIcyMetadata(ParsableByteArray parsableByteArray) {
            ...
            TrackOutput trackOutput = (TrackOutput) Preconditions.checkNotNull(this.icyTrackOutput);
        ...
        private DataSpec buildDataSpec(long j, String str) {
            Map<String, String> mapBuildKeepingLast = ProgressiveMediaPeriod.ICY_METADATA_HEADERS;
            if (str != null && !str.startsWith("W/")) {
                mapBuildKeepingLast = ImmutableMap.builder().putAll(mapBuildKeepingLast).put(HttpHeaders.IF_RANGE, str).buildKeepingLast();
            }
            return new DataSpec.Builder().setUri(this.uri).setPosition(j).setKey(ProgressiveMediaPeriod.this.customCacheKey).setFlags(6).setHttpRequestHeaders(mapBuildKeepingLast).build();
        }
    ...
    private static class ControlledTrackOutput extends ForwardingTrackOutput {
        ...
        public int sampleData(DataReader dataReader, int i, boolean z, int i2) throws IOException {
            return getCurrentOutput().sampleData(dataReader, i, z, i2);
        }

        @Override // androidx.media3.extractor.ForwardingTrackOutput, androidx.media3.extractor.TrackOutput
        public void sampleData(ParsableByteArray parsableByteArray, int i) {
            getCurrentOutput().sampleData(parsableByteArray, i);
        }
